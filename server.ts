...
function getDefaultProfile(resumeText: string = "", candidateName: string = "") {
  ...
  const isAnanya = candidateName.includes("Ananya") || resumeText.includes("Ananya Iyer") || resumeText.includes("Bridgepoint");
  ...
  if (isRohan) {
    ...
    // Custom Candidate dynamically synthesized profile
    const name = candidateName.trim() || "Custom Candidate";
    return {
      id: "custom_candidate",
      name,
      title: "Full-Stack / AI Systems Engineer",
      yearsOfExperience: 4.5,
      education: "B.S. / B.Tech Computer Science",
      skills: ["TypeScript", "Python", "FastAPI", "React", "Vector Databases", "LLM APIs", "Docker"],
      keyClaims: [
        {
          claim: "Built end-to-end production AI pipeline and verified system resiliency",
          source: "Custom Application Details",
          verifiedInTranscript: true,
          evidenceQuote: "Q1: 'Designed modular architecture with graceful retries, error boundaries, and telemetry.'"
        },
        {
          claim: "High team collaboration, transparent communication and incident ownership",
          source: "Custom Interview Session",
          verifiedInTranscript: true,
          evidenceQuote: "Q2: 'Proactively shared learnings in postmortems and credited team contributions.'"
        },
        {
          claim: "Commitment to engineering safety and continuous benchmarking",
          source: "Custom Interview Session",
          verifiedInTranscript: true,
          evidenceQuote: "Q4: 'Instituted evaluation sets and test suites before rolling changes to production.'"
        }
      ],
      summary: `Evaluated live candidate ${name} through interactive multi-agent panel interview, synthesizing direct answers against core technical, cultural, and operational criteria.`,
      missingOrUnclearInfo: [
        "Long-term multi-year scale metrics in current architecture",
        "Formal third-party backchannel reference corroboration"
      ]
    };
  }
}
...
function getDefaultIndependentOpinions(candidateName: string = "", resumeText: string = "") {
  ...
  const isAnanya = candidateName.includes("Ananya") || resumeText.includes("Ananya") || resumeText.includes("Bridgepoint");
  ...
  if (isRohan) {
    ...
    // Custom Candidate independent opinions
    ...
    return {
      technical: {
        ...
        confidence: 88,
        verdict: "HIRE",
        corePerspective: `Demonstrates solid architectural clarity, practical grasp of async state pipelines, and defensive coding practices.`,
        strengths: [
          { point: "Clear system decomposition and async architecture", evidence: `Q1: 'Architected modular pipelines with retry boundaries, logging, and state decoupling.'` },
          { point: "Pragmatic debugging and evaluation hygiene", evidence: `Q4: 'Instituted test harnesses and validation benchmarks before deployments.'` }
        ],
        concerns: [
          { point: "Continuous load benchmarking depth", evidence: `Q1: 'Scalability under sustained 10k RPS requires further profiling.'`, severity: "low" }
        ],
        missingInformationIdentified: ["Specific distributed caching benchmarks under high concurrent queries"],
        isolatedProof: `Evaluated candidate ${name} strictly from interview transcript technical responses.`
      },
      hr_culture: {
        agentRole: "hr_culture",
        agentName: "Elena Rostova",
        ...
        confidence: 92,
        verdict: "STRONG_HIRE",
        corePerspective: `High emotional intelligence, transparent ownership of previous mistakes, and collaborative mindset.`,
        strengths: [
          { point: "Intellectual honesty and credit sharing", evidence: `Q2: 'Credited team members and conducted transparent postmortems without blame.'` },
          { point: "Strong communication and constructive feedback receptivity", evidence: `Q2: 'Maintained open dialogue and adapted approaches based on team needs.'` }
        ],
        concerns: [],
        missingInformationIdentified: [],
        isolatedProof: `Evaluated behavioral responses and interpersonal alignment for ${name}.`
      },
      hiring_manager: {
        agentRole: "hiring_manager",
        agentName: "Marcus Sterling",
        ...
        confidence: 86,
        verdict: "HIRE",
        corePerspective: `Well-rounded contributor with quick onboarding potential, realistic production expectations, and low retention risk.`,
        strengths: [
          { point: "Proactive ramp-up strategy", evidence: `Q3: 'Prepared to pair on existing codebases and establish operational rhythms in first 30 days.'` },
          { point: "Production on-call readiness", evidence: `Q3: 'Comfortable with on-call rotations and maintaining incident runbooks.'` }
        ],
        concerns: [
          { point: "Domain-specific framework onboarding", evidence: `Q3: 'Will require 1-2 weeks ramp on proprietary internal infrastructure.'`, severity: "low" }
        ],
        missingInformationIdentified: ["Specific compensation and start date alignment"],
        isolatedProof: `Assessed business impact, team velocity ROI, and delivery reliability.`
      },
      skeptic: {
        agentRole: "skeptic",
        agentName: "Devon Cross",
        ...
        confidence: 85,
        verdict: "LEAN_HIRE",
        corePerspective: `Substantiated key experience claims under adversarial probing; answered without evasiveness or resume inflation.`,
        strengths: [
          { point: "Honest calibration of personal contributions", evidence: `Q4: 'Distinguished individual contributions from shared architectural design.'` },
          { point: "Grounded responses without hype buzzwords", evidence: `Q4: 'Provided realistic limitations of current implementation.'` }
        ],
        concerns: [
          { point: "Edge-case validation proof", evidence: `Q4: 'Third-party scale logs should be validated during reference checks.'`, severity: "low" }
        ],
        missingInformationIdentified: ["Direct peer reference verification"],
        isolatedProof: `Red-team fact verification completed for ${name}.`
      }
    };
  }
}
...
function getDefaultDebate(candidateName: string = "") {
  ...
  const isAnanya = candidateName.includes("Ananya") || candidateName.includes("Bridgepoint");
      ...
      debateMessages: [
        {
          ...
          targetName: "Dr. Alex Vance (Technical)",
          messageType: "challenge",
          content: `Alex, we need to verify ${name}'s claim regarding production scalability. How confident are we that the candidate's architecture has been battle-tested under sustained concurrency?`,
          citedQuote: "Q1: 'Designed modular architecture with graceful retries, error boundaries, and telemetry.'",
          citedSource: "Interview Transcript Q1"
        },
        {
          id: "msg_2",
          round: 1,
          ...
          targetName: "Devon Cross (Skeptic)",
          messageType: "rebuttal",
          content: `Devon, ${name}'s answers demonstrate concrete defensive patterns—specifically separating the state machine from the inference worker and implementing pre-flight validation harnesses. That's sound production hygiene.`,
          citedQuote: "Q1 & Q4: 'Instituted test harnesses and validation benchmarks before deployments.'",
          citedSource: "Interview Transcript Q1 & Q4"
        },
        {
          id: "msg_3",
          round: 2,
          ...
          targetName: "Marcus Sterling (Hiring Manager)",
          messageType: "synthesis",
          content: `What stands out most about ${name} is the balanced self-awareness and collaborative incident resolution. In Q2, the candidate clearly credited peers and emphasized transparent blameless postmortems.`,
          citedQuote: "Q2: 'Credited team members and conducted transparent postmortems without blame.'",
          citedSource: "Interview Transcript Q2"
        },
        {
          id: "msg_4",
          round: 2,
          ...
          targetName: "Elena Rostova (HR/Culture)",
          messageType: "concession",
          content: `Agreed. Given the solid engineering baseline and strong communication ownership, I am comfortable with immediate team placement. I am moving my score from 8.0 to 8.4.`,
          citedQuote: "Q3: 'Prepared to pair on existing codebases and establish operational rhythms in first 30 days.'",
          citedSource: "Interview Transcript Q3",
          stanceShiftBefore: 8.0,
          stanceShiftAfter: 8.4,
          shiftReason: "Confirmed that transparent communication and strong engineering fundamentals ensure rapid team onboarding."
        }
      ],
      stanceShifts: [
        {
          ...
          delta: 0.4,
          triggerMessageId: "msg_3",
          triggerQuote: "Q2: 'Credited team members and conducted transparent postmortems without blame.'",
          reasonForChange: `HR highlighted ${name}'s exceptional peer collaboration and transparent postmortem ownership.`,
          shiftMomentDescription: "Hiring Manager boosted evaluation score upon reviewing strong collaborative culture signals."
        }
      ]
    };
  }
...
function getDefaultDecision(candidateName: string = "") {
  ...
  if (isRohan) {
    ...
    // Custom Candidate Final Decision
    const name = candidateName.trim() || "Candidate";
    return {
      ...
      scoringBreakdown: {
        ...
        hiringManagerWeight: 0.25,
        skepticWeight: 0.15,
        explanation: `Balanced synthesis across architectural discipline (30%), cultural alignment and honest communication (30%), and hiring manager delivery ROI (25%).`
      },
      executiveSummary: `Following deliberation by the 4-agent committee, the panel officially recommends a HIRE decision for ${name}. The candidate demonstrated sound architectural clarity, direct accountability for engineering decisions, and strong alignment with organizational quality standards.`,
      keyStrengths: [
        "Clear grasp of modular system architecture, exception handling, and observability",
        "Transparent communication style with proactive credit attribution and blameless postmortems",
        "Disciplined approach to pre-deployment testing and validation sets",
        "Constructive response to adversarial cross-examination"
      ],
      keyRisksAndConcerns: [
        "Onboarding buffer needed to master proprietary codebase conventions and domain tooling"
      ],
      unresolvedDisagreements: [
        {
          topic: "Scale verification vs Rapid delivery onboarding",
          agentsInvolved: [
            { role: "skeptic", stance: "Recommended confirming high-concurrency benchmarks during references." },
            { role: "technical", stance: "Satisfied with candidate's conceptual and practical architectural answers." }
          ],
          impactOnRole: "Low operational risk with initial paired code reviews.",
          mitigationSuggestion: "Institute 2-week pairing on production deploy pipelines."
        }
      ],
      hiringConditions: [
        "Standard reference check confirming previous team contributions",
        "Initial 30-day ramp-up milestone focused on core architecture and monitoring harnesses"
      ],
      missingDataNotes: [
        "Peer references from prior direct technical lead"
      ]
    };
  }
}
...
// 6. Custom Interactive Interview: Question Generation Endpoint
app.post("/api/interview/generate-questions", async (req, res) => {
  try {
    const { candidateName = "Candidate", jobTitle = "Senior AI Engineer", resumeText = "", jobDescription = "" } = req.body;
    const ai = getAI();

    if (ai) {
      const prompt = `You are the lead coordinator for an autonomous 4-agent hiring panel:
- Technical Architect (Dr. Alex Vance)
- HR & Culture Specialist (Elena Rostova)
- Hiring Manager / Director of Engineering (Marcus Sterling)
- Skeptic & Red-Team Fact Checker (Devon Cross)

Given the following Candidate Resume and Target Job Description:
Candidate Name: ${candidateName}
Target Job Title: ${jobTitle}
Job Description: ${jobDescription || "Design, scale, and reliably operate production AI systems, LLM workflows, and backend microservices."}
Resume / Experience Summary:
${resumeText || "5+ years backend and fullstack software development with Python, TypeScript, microservices, and AI integrations."}

Generate exactly 5 targeted, high-impact interview questions, each assigned to one of the panel members:
1. Technical Architecture & System Design (Dr. Alex Vance)
2. Behavioral Ownership, Failures, & Team Collaboration (Elena Rostova)
3. Engineering Delivery, Ramp-Up Speed, & Reliability (Marcus Sterling)
4. Red-Team Fact Check on Specific Resume Metrics / Claims (Devon Cross)
5. Live Scenario & Critical Trade-Off Decision (Joint Panel)`;
        ...
        properties: {
          questions: {
            ...
            items: {
              ...
              required: ["id", "interviewerRole", "interviewerName", "category", "questionText", "evaluationFocus"]
            }
          }
        },
        required: ["questions"]
      ...
      questions: [
        {
          ...
          interviewerName: "Dr. Alex Vance (Technical Architect)",
          category: "System Design & Architecture",
          questionText: `Could you walk us through the most complex distributed system or AI pipeline you designed, specifically how you handled async state, backpressure, and fault tolerance?`,
          evaluationFocus: "Assesses architectural depth, modularity, retry semantics, and clean decoupled state."
        },
        {
          id: "q2",
          interviewerRole: "hr_culture",
          interviewerName: "Elena Rostova (VP of People & Culture)",
          category: "Integrity & Team Postmortems",
          questionText: `Tell us about a time when a system you owned failed or caused an outage in production. How did you communicate the issue and how was credit/blame handled across the team?`,
          evaluationFocus: "Evaluates blameless postmortem mindset, intellectual honesty, and psychological safety."
        },
        {
          id: "q3",
          interviewerRole: "hiring_manager",
          interviewerName: "Marcus Sterling (Director of Engineering)",
          category: "Execution Velocity & Onboarding",
          questionText: `How would you structure your first 30 to 60 days on our engineering team to balance learning our proprietary stack with shipping meaningful production value?`,
          evaluationFocus: "Measures ramp-up strategy, self-sufficiency, pairing intuition, and realistic delivery velocity."
        },
        {
          id: "q4",
          interviewerRole: "skeptic",
          interviewerName: "Devon Cross (Senior Red-Team Reviewer)",
          category: "Adversarial Fact Check",
          questionText: `Looking at your experience, what is the single biggest technical limitation or unverified assumption in the systems you've built that wouldn't hold up under 10x traffic?`,
          evaluationFocus: "Tests self-awareness, resistance to resume inflation, and candid technical calibration."
        },
        {
          id: "q5",
          interviewerRole: "technical",
          interviewerName: "Joint Panel Simulation",
          category: "Production Trade-Offs",
          questionText: `When faced with a severe latency spike vs output accuracy trade-off during a peak traffic event, what immediate defensive mechanisms do you trigger first?`,
          evaluationFocus: "Verifies incident response reflexes, caching hierarchies, and graceful degradation."
        }
      ]
    });
  } catch (error: any) {
...
// 7. Custom Interactive Interview: AI Answer Simulation Endpoint
app.post("/api/interview/simulate-answers", async (req, res) => {
  try {
    const { candidateName = "Candidate", resumeText = "", questions = [] } = req.body;
    const ai = getAI();

    if (ai && Array.isArray(questions) && questions.length > 0) {
      const prompt = `You are generating realistic, authentic candidate interview answers for ${candidateName} based on their background:
Candidate Resume/Background:
${resumeText || "Experienced Software Engineer with strong background in backend systems, APIs, cloud deployments, and collaborative engineering."}

Questions to answer:
${JSON.stringify(questions, null, 2)}

Provide authentic, detailed first-person answers from ${candidateName}'s perspective that reflect honest engineering practices, real-world trade-offs, and balanced self-awareness.`;
    ...
    const answers = (questions || []).map((q: any, idx: number) => {
      const fallbacks = [
        "In our core pipeline, we decoupled the orchestrator from individual worker queues using RabbitMQ and Redis caching. Every failure is categorized into transient retries (exponential backoff capped at 3) versus terminal errors that route to an operator alert table with complete request payloads.",
        "We experienced an incident where a faulty regex parser caused worker memory leaks under anomalous inputs. I led the emergency rollback, documented our timeline in the postmortem, and worked with the team to add automated fuzz-testing to our pre-merge CI pipeline.",
        "During my first month, I prioritize diving into existing runbooks and pair programming on smaller bug tickets. This allows me to understand the production release patterns and team norms before taking on architecture refactoring.",
        "The biggest bottleneck in our previous architecture was synchronous database indexing during batch ingestion. While it worked for our initial dataset, scaling by 10x required moving to asynchronous event streaming and partitioned tables.",
        "During peak load spikes, we enforce circuit breakers with tiered fallback responses and stale-while-revalidate caches, guaranteeing sub-150ms responses to upstream clients while shielding backend databases."
      ];
      return {
        questionId: q.id || `q${idx + 1}`,
        answerText: fallbacks[idx % fallbacks.length]
      };
    });

    return res.json({ answers });
  } catch (error: any) {
    return res.status(500).json({ error: "Failed to simulate interview answers" });
  }
});
