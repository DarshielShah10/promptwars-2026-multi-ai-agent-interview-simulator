...
import { CustomCandidateInterviewPanel } from './components/CustomCandidateInterviewPanel';
import { 
  CheckCircle2, 
  AlertCircle, 
  Loader2, 
...
export default function App() {
  ...
  const [showCustomIntakeForm, setShowCustomIntakeForm] = useState<boolean>(true);
  ...
  // Pipeline for Custom Candidate
  const handleRunCustomPipeline = async (customData: {
    candidateId: string;
    candidateName: string;
    jobDescription: string;
    resumeText: string;
    ...
    setActiveCandidate('custom');
    setShowCustomIntakeForm(false);
    const targetName = customData.candidateName;
    ...
    try {
      ...
      const profRes = await fetch('/api/evaluate/profile', {
        ...
        body: JSON.stringify({
          ...
          resumeText: customData.resumeText,
          transcriptText: customData.transcriptText,
          jobDescription: customData.jobDescription,
          forceLive: true
        })
      });
      ...
      console.warn('Error running custom pipeline:', err);
    } finally {
  ...
  const handleRunFullPipeline = async () => {
    if (activeCandidate === 'custom') {
      if (dataCustom.resumeText && dataCustom.transcriptText) {
        handleRunCustomPipeline({
          candidateId: 'custom_candidate',
          candidateName: dataCustom.candidateName || 'Custom Candidate',
          jobDescription: dataCustom.jobDescription || JOB_DESCRIPTION,
          resumeText: dataCustom.resumeText,
          transcriptText: dataCustom.transcriptText
        });
      } else {
        setShowCustomIntakeForm(true);
      }
      return;
    }
  ...
  const currentData = activeCandidate === 'candidate_a' 
    ? dataA 
    : activeCandidate === 'candidate_b' 
    ? dataB 
    : dataCustom;

  const currentCandidateName = activeCandidate === 'candidate_a'
    ? 'Rohan Malhotra'
    : activeCandidate === 'candidate_b'
    ? 'Ananya Iyer'
    : dataCustom.candidateName || 'Custom Candidate';
  ...
  return (
    <div className="min-h-screen text-slate-900 flex flex-col font-sans selection:bg-indigo-500 selection:text-white relative">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {/* Custom Intake & Live Interview Portal */}
            ...
            <div className="flex justify-between items-center mb-3">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-base font-bold text-slate-900">
                  Custom Candidate Interview Panel
                </h2>
                {dataCustom.profile && (
                  <span className="text-xs px-2.5 py-0.5 rounded-full bg-emerald-100 text-emerald-800 font-semibold border border-emerald-300">
                    Deliberation Ready ({dataCustom.candidateName})
                  </span>
                )}
              </div>

              <button
                id="toggle-custom-intake-btn"
                onClick={() => setShowCustomIntakeForm(!showCustomIntakeForm)}
                className="text-xs px-3 py-1.5 rounded-lg border border-slate-300 bg-white hover:bg-slate-50 text-slate-700 font-semibold flex items-center gap-1.5 transition shadow-2xs"
              >
                <Edit3 className="w-3.5 h-3.5 text-indigo-600" />
                {showCustomIntakeForm ? 'Hide Intake Form' : 'Edit Candidate Credentials / Re-Take Interview'}
              </button>
            </div>

            {showCustomIntakeForm && (
              <CustomCandidateInterviewPanel
                onStartCustomEvaluation={handleRunCustomPipeline}
          ...
          <>
            ...
            {activeStage === 'independent' && currentData.opinions && (
              <IndependentAgentsView 
                ...
                candidateProfile={currentData.profile || undefined}
            ...
            {/* Empty state prompt if custom candidate has not been evaluated yet */}
            {activeCandidate === 'custom' && !currentData.profile && !showCustomIntakeForm && (
              <div className="p-10 text-center bg-white rounded-xl border border-slate-200 shadow-sm space-y-3">
                ...
                <h3 className="text-base font-bold text-slate-800">No Evaluation Loaded for Custom Candidate</h3>
                <p className="text-xs text-slate-500 max-w-md mx-auto">
                  Use the Intake Form above to enter your details, conduct or simulate the live interview, and start the 4-agent committee deliberation.
                </p>
                <button
                  onClick={() => setShowCustomIntakeForm(true)}
                  className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs font-bold shadow transition"
                >
                  Open Candidate Intake Portal
                </button>
              </div>
            )}
          </>
